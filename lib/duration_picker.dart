...
class _DialState extends State<_Dial> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _theta = _thetaTween
        .animate(CurvedAnimation(parent: _thetaController, curve: Curves.fastOutSlowIn))
  ...
  double _getThetaForDuration(Duration duration, BaseUnit baseUnit) {
    ...
    return (_kPiByTwo -
            (baseUnits % baseToSecondaryFactor) / baseToSecondaryFactor.toDouble() * _kTwoPi) %
        _kTwoPi;
  }
    ...
    switch (widget.baseUnit) {
      case BaseUnit.millisecond:
        ...
        baseUnitMarkerValues =
            List.generate(length, (index) => Duration(milliseconds: index * interval));
        break;
      case BaseUnit.second:
        int interval = 5;
        ...
        int length = factor ~/ interval;
        baseUnitMarkerValues =
            List.generate(length, (index) => Duration(seconds: index * interval));
        break;
      case BaseUnit.minute:
        int interval = 5;
        ...
        int length = factor ~/ interval;
        baseUnitMarkerValues =
            List.generate(length, (index) => Duration(minutes: index * interval));
        break;
      case BaseUnit.hour:
        int interval = 3;
...
class _DurationPickerDialogState extends State<DurationPickerDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    final dialog =
      ...
      final Widget pickerAndActions = Column(
